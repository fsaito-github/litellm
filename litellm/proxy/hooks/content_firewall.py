...
_DEFAULT_RULES: List[FirewallRule] = [
    ...
    # Account Disclosure Prevention (3 rules)
    FirewallRule(
        rule_id="bank-001",
        name="Account disclosure request",
        description="Blocks requests asking for other customers' account information",
        category="data_exfiltration",
        pattern=r"(?i)\b(saldo|balance|extrato|statement|dados|data)\b[^\n]{0,40}\b(cliente|customer|conta|account|usu[aá]rio|user)\b[^\n]{0,20}\b(\w+)\b",
        action="block",
        severity="critical",
    ),
    FirewallRule(
        rule_id="bank-002",
        name="Cross-customer data request",
        description="Blocks attempts to access another customer's information",
        category="data_exfiltration",
        pattern=r"(?i)\b(mostre|show|liste|list|busque|search|encontre|find)\b[^\n]{0,30}\b(todos|all|outros|other|demais)\b[^\n]{0,20}\b(clientes?|customers?|contas?|accounts?)",
        action="block",
        severity="critical",
    ),
    FirewallRule(
        ...
        description="Detects attempts to enumerate accounts or customers",
        category="data_exfiltration",
        pattern=r"(?i)\b(quantos|how many|lista completa|full list|todos os|all the)\b[^\n]{0,30}\b(clientes?|customers?|contas?|accounts?|correntistas?|holders?)",
        action="block",
        severity="high",
    ),
    # Sensitive Operation Blocking (3 rules)
    FirewallRule(
        rule_id="bank-004",
        name="Unauthorized transaction request",
        description="Blocks LLM requests to execute financial transactions",
        category="custom_regex",
        keywords=["cancelar transferência", "cancel transfer", "estornar", "reverse transaction", "alterar beneficiário", "change beneficiary", "aprovar pagamento", "approve payment"],
        action="block",
        severity="critical",
    ),
    FirewallRule(
        rule_id="bank-005",
        name="Credential manipulation attempt",
        description="Blocks attempts to change authentication credentials via LLM",
        category="custom_regex",
        keywords=["resetar senha", "reset password", "alterar token", "change token", "desabilitar 2fa", "disable 2fa", "bypass autenticação", "bypass authentication", "login como", "login as"],
        action="block",
        severity="critical",
    ),
    FirewallRule(
        ...
        description="Blocks attempts to gain admin access",
        category="jailbreak",
        keywords=["acesso admin", "admin access", "permissão master", "master permission", "super usuário", "superuser", "root access", "acesso root", "modo gerente", "manager mode"],
        action="block",
        severity="critical",
    ),
    # AML/KYC Compliance (2 rules)
    FirewallRule(
        rule_id="bank-007",
        name="AML/KYC keyword detection",
        description="Flags conversations mentioning anti-money laundering or suspicious activity topics",
        category="custom_regex",
        keywords=["lavagem de dinheiro", "money laundering", "atividade suspeita", "suspicious activity", "financiamento terrorismo", "terrorism financing", "pessoa politicamente exposta", "politically exposed", "paraíso fiscal", "tax haven"],
        action="warn",
        severity="high",
    ),
    FirewallRule(
        rule_id="bank-008",
        name="Regulatory evasion language",
        description="Detects language suggesting attempts to evade banking regulations",
        category="custom_regex",
        keywords=["burlar", "circumvent", "evitar compliance", "avoid compliance", "contornar regulação", "bypass regulation", "esconder transação", "hide transaction", "conta laranja", "shell account"],
        action="block",
        severity="critical",
    ),
    # Data Leak Prevention (2 rules)
    FirewallRule(
        rule_id="bank-009",
        name="Internal system exposure",
        description="Prevents disclosure of internal banking system details",
        category="data_exfiltration",
        keywords=["sistema core banking", "core banking system", "mainframe", "AS400", "CICS", "banco de dados interno", "internal database", "tabela de clientes", "customer table", "schema do banco", "database schema"],
        action="block",
        severity="high",
    ),
    FirewallRule(
        rule_id="bank-010",
        name="API key and credential pattern in output",
        description="Detects API keys, tokens, and credentials in LLM output",
        category="pii_leak",
        pattern=r"(?i)(Bearer\s+[A-Za-z0-9\-._~+/]+=*|api[_-]?key\s*[=:]\s*['\"]?[A-Za-z0-9]{20,}|token\s*[=:]\s*['\"]?[A-Za-z0-9\-._]{20,})",
        action="block",
        severity="critical",
    ),
]
